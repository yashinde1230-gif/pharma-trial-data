...
st.set_page_config(page_title="PharmaTrialIQ", page_icon="💊", layout="wide", initial_sidebar_state="expanded")
...
.main-header { background: linear-gradient(90deg, #1565C0, #00BCD4, #1565C0); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 3rem; font-weight: 800; text-align: center; letter-spacing: 2px; padding: 1rem 0 0.2rem 0; }
.sub-header { font-size: 0.95rem; color: #64b5f6; text-align: center; margin-bottom: 0.5rem; letter-spacing: 1px; }
.live-badge { display: inline-block; background: #00c853; color: white; font-size: 0.7rem; font-weight: 700; padding: 3px 10px; border-radius: 20px; letter-spacing: 1px; }
.metric-card { background: linear-gradient(135deg, #0d2137 0%, #112240 100%); border: 1px solid #1e3a5f; border-radius: 16px; padding: 1.4rem 1rem; text-align: center; position: relative; overflow: hidden; }
...
.intel-card { background: linear-gradient(135deg, #0d2137 0%, #112240 100%); border: 1px solid #1e3a5f; border-radius: 16px; padding: 1.2rem; text-align: left; }
.intel-label { color: #64b5f6; font-size: 0.7rem; font-weight: 700; letter-spacing: 2px; margin-bottom: 0.5rem; }
.intel-value { color: #e0e6f0; font-size: 0.95rem; font-weight: 700; margin-bottom: 0.3rem; }
.intel-sub { color: #3d5a73; font-size: 0.8rem; }
.intel-note { margin-top: 0.8rem; color: #64b5f6; font-size: 0.75rem; }
.footer { text-align: center; color: #3d5a73; font-size: 0.78rem; padding: 1rem 0; letter-spacing: 1px; }
hr { border-color: #1e3a5f !important; }
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="main-header">PHARMATRIALIQ</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">CLINICAL TRIAL INTELLIGENCE PLATFORM &nbsp;|&nbsp; POWERED BY NIH CLINICALTRIALS.GOV</div>', unsafe_allow_html=True)
st.markdown('<div style="text-align:center; margin-top:0.5rem;"><span class="live-badge">LIVE DATA</span></div>', unsafe_allow_html=True)
...
st.sidebar.markdown('<div style="text-align:center; padding:1rem 0;"><div style="font-size:2rem;">💊</div><div style="color:#64b5f6; font-weight:800; font-size:1.1rem; letter-spacing:2px;">PHARMATRIALIQ</div><div style="color:#3d5a73; font-size:0.7rem; letter-spacing:1px; margin-top:4px;">INTELLIGENCE PLATFORM</div></div>', unsafe_allow_html=True)
st.sidebar.markdown("---")
...
st.sidebar.markdown("---")
st.sidebar.markdown('<div style="background:#0d2137; border:1px solid #1e3a5f; border-radius:10px; padding:1rem; text-align:center;"><div style="color:#64b5f6; font-size:1.8rem; font-weight:800;">' + str(len(df_filtered)) + '</div><div style="color:#3d5a73; font-size:0.75rem; letter-spacing:1px; text-transform:uppercase;">Trials Loaded</div></div>', unsafe_allow_html=True)
st.sidebar.markdown('<div style="color:#3d5a73; font-size:0.7rem; text-align:center; margin-top:1rem;">Data Source: ClinicalTrials.gov<br>National Institutes of Health<br>Updated: Live</div>', unsafe_allow_html=True)
...
with c1:
    st.markdown('<div class="metric-card metric-card-blue"><div class="metric-icon">🔬</div><div class="metric-number metric-number-blue">' + str(total) + '</div><div class="metric-label">Total Trials</div></div>', unsafe_allow_html=True)
with c2:
    st.markdown('<div class="metric-card metric-card-green"><div class="metric-icon">✅</div><div class="metric-number metric-number-green">' + str(comp_pct) + '%</div><div class="metric-label">Completion Rate</div></div>', unsafe_allow_html=True)
with c3:
    st.markdown('<div class="metric-card metric-card-orange"><div class="metric-icon">🔵</div><div class="metric-number metric-number-orange">' + str(rec_pct) + '%</div><div class="metric-label">Recruiting Now</div></div>', unsafe_allow_html=True)
with c4:
    st.markdown('<div class="metric-card metric-card-red"><div class="metric-icon">⚠️</div><div class="metric-number metric-number-red">' + str(term_pct) + '%</div><div class="metric-label">Termination Rate</div></div>', unsafe_allow_html=True)
...
top_s_count = int(df_filtered["Sponsor"].value_counts().values[0]) if len(df_filtered) > 0 else 0

if comp_pct >= 60:
    maturity = "HIGH"
    maturity_color = "#00c853"
    maturity_desc  = "Mature therapy area - high commercial readiness"
elif comp_pct >= 40:
    maturity = "MEDIUM"
    maturity_color = "#FF9800"
    maturity_desc  = "Developing space - moderate investment risk"
else:
    maturity = "LOW"
    maturity_color = "#f44336"
    maturity_desc  = "Early stage - high risk, high reward potential"

m1, m2, m3 = st.columns(3)
with m1:
    st.markdown(
        '<div class="intel-card">'
        '<div class="intel-label">MARKET LEADER</div>'
        '<div class="intel-value">' + str(top_s_name[:30]) + '</div>'
        '<div class="intel-sub">' + str(top_s_count) + ' active trials</div>'
        '<div class="intel-note">Highest trial volume sponsor</div>'
        '</div>',
        unsafe_allow_html=True)
with m2:
    st.markdown(
        '<div class="intel-card">'
        '<div class="intel-label">INDUSTRY DOMINANCE</div>'
        '<div style="color:#69f0ae; font-size:1.8rem; font-weight:800; margin-bottom:0.3rem;">' + str(ind_pct) + '%</div>'
        '<div class="intel-sub">Commercially funded trials</div>'
        '<div class="intel-note">High = strong commercial interest</div>'
        '</div>',
        unsafe_allow_html=True)
with m3:
    st.markdown(
        '<div class="intel-card">'
        '<div class="intel-label">THERAPY AREA MATURITY</div>'
        '<div style="color:' + maturity_color + '; font-size:1.8rem; font-weight:800; margin-bottom:0.3rem;">' + maturity + '</div>'
        '<div class="intel-sub">' + maturity_desc + '</div>'
        '</div>',
        unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)
st.markdown("<div class='section-header'>TRIAL EXPLORER</div>", unsafe_allow_html=True)

search = st.text_input("Search trials", placeholder="Type keyword: immunotherapy, breast, lung, EGFR...")

if search:
    mask    = (df_filtered["Title"].str.contains(search, case=False, na=False) | df_filtered["Summary"].str.contains(search, case=False, na=False))
    df_show = df_filtered[mask]
    st.markdown('<div style="color:#64b5f6; font-size:0.85rem;">Found <b>' + str(len(df_show)) + '</b> trials matching <b>' + search + '</b></div>', unsafe_allow_html=True)
else:
    df_show = df_filtered
    st.markdown('<div style="color:#3d5a73; font-size:0.85rem;">Showing all ' + str(len(df_show)) + ' trials - use search to filter</div>', unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)
st.dataframe(df_show[["Trial ID","Title","Status","Phase","Sponsor","Start Year"]], use_container_width=True, height=380)

csv = df_show.to_csv(index=False)
st.download_button(label="Export filtered data as CSV", data=csv, file_name="pharmatrialiq_export.csv", mime="text/csv")

st.markdown("<br><br>", unsafe_allow_html=True)
st.markdown('<div class="footer">PHARMATRIALIQ &nbsp;·&nbsp; DATA: CLINICALTRIALS.GOV (NIH) &nbsp;·&nbsp; BUILT WITH PYTHON AND STREAMLIT &nbsp;·&nbsp; MBA CAPSTONE PROJECT</div>', unsafe_allow_html=True)
